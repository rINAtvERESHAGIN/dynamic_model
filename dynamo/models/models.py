...
"""Provides the base test_model of dynamic model schema classes.

Abstract test_model should be subclassed to provide extra functionality, but they
are perfectly usable without adding any additional fields.

`AbstractModelSchema` -- base model that defines dynamic test_model
`AbstractFieldSchema` -- base model for defining fields to use on dynamic test_model
`DynamicModelField`   -- through model for attaching fields to dynamic test_model
"""
from django.db import models
...
from dynamo import exceptions
from dynamo.schema import ModelSchemaEditor, FieldSchemaEditor
from dynamo.factory import ModelFactory, FieldFactory
...
class MyModel(AbstractModelSchema):
    pass


class FieldsForModel(AbstractFieldSchema):
    pass


class InstanceTable(models.Model):
    my_model = models.ForeignKey(MyModel, on_delete=models.CASCADE)
    fields_for_model = models.ForeignKey(FieldsForModel, on_delete=models.CASCADE)

    # test = ModelFieldSchema.unique
    # test = ModelFieldSchema.max_length
    # test = ModelFieldSchema.null
